...
const ReportTable: React.FC<ReportTableProps> = ({ logs, loading }) => {
  ...
  const renderCell = (row: ScanLog, key: keyof ScanLog) => {
    ...
    // Scan Time — parse the DB scan_time TIMESTAMPTZ and show Date + Time
    if (key === "scan_time") {
      if (!value) return "-";
        ...
        .map((roundKey) => {
          ...
          return (
            ...
            >
              ...
              {/* Table */}
              <table className="min-w-full divide-y divide-slate-200/50 bg-white/50 backdrop-blur-sm rounded-xl overflow-hidden">
