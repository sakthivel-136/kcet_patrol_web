...
;
// ================= Component =================
const PatrolReportPDF = ({ logs, shifts, campusCode, campusName, campusAddress, reportDate, generatedBy })=>{
    _s();
    ...
    const generatePDF = ()=>{
        ...
        if (dates.length > 1) {
            ...
            let grandY = 72;
            const isTimeInShift = (time, start, end)=>{
            ...
            const shiftsToProcess = shifts && shifts.length > 0 ? shifts : [
                {
                    shift_name: "ALL DAY",
                    start_time: "00:00:00",
                    end_time: "24:00:00"
            ...
            shiftsToProcess.forEach((shift)=>{
                const shiftLogs = shiftsToProcess.length === 1 && shift.shift_name === "ALL DAY" ? logs : logs.filter((l)=>{
                    const roundTime = __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$components$2f$reports$2f$roundtime$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["ROUND_TIMES"][l.round];
                ...
                shiftLogs.forEach((r)=>{
                    ...
                    if (status === 'SUCCESS' || status === 'MISSED') {
                        ...
                        guardsList.forEach((g)=>{
                            if (!grandGuardMap[g]) grandGuardMap[g] = {
                                scanned: 0,
                                missed: 0
                            };
                ...
                const grandSummaryRows = Object.entries(grandGuardMap).filter(([name, stats])=>name !== 'Unknown' || stats.scanned > 0 || stats.missed > 0).sort(([, aStats], [, bStats])=>{
                ...
                if (grandSummaryRows.length > 0) {
                    ...
                    grandY = doc.lastAutoTable.finalY + 12;
                }
            });
        ...
        dates.forEach((d, dateIdx)=>{
            ...
            Object.keys(byRound).map(Number).sort((a, b)=>a - b).forEach((round)=>{
                ...
                doc.setFontSize(12);
                doc.setTextColor(0, 0, 150);
                doc.text(`ROUND ${round} (${time?.start || "-"} - ${time?.end || "-"})`.toUpperCase(), 14, y);
                y += 6;
                // ================= Rows =================
                const rows = byRound[round].map((l)=>{
                    const status = normalizeStatus(l.status);
                    const timeStr = l.scan_time ? new Date(l.scan_time.replace(' ', 'T')).toLocaleTimeString("en-IN", {
                        hour: "2-digit",
                        minute: "2-digit"
                    }) : "-";
                    return [
            ...
            shiftsToProcess.forEach((shift)=>{
                const shiftLogs = shiftsToProcess.length === 1 && shift.shift_name === "ALL DAY" ? dayLogs : dayLogs.filter((l)=>{
                    const roundTime = __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$components$2f$reports$2f$roundtime$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["ROUND_TIMES"][l.round];
                    return roundTime && isTimeInShift(roundTime.start, shift.start_time, shift.end_time);
                });
                if (shiftLogs.length === 0) return;
                ...
                const dailySummaryRows = Object.entries(dailyGuardMap).filter(([name, stats])=>name !== 'Unknown' || stats.scanned > 0 || stats.missed > 0).sort(([, aStats], [, bStats])=>{
                    const aTotal = aStats.scanned + aStats.missed;
                    const bTotal = bStats.scanned + bStats.missed;
                    const aPct = aTotal > 0 ? aStats.scanned / aTotal : 0;
                    const bPct = bTotal > 0 ? bStats.scanned / bTotal : 0;
...
//# sourceMappingURL=app_components_reports_580d41e0._.js.map